...
app = FastAPI(title="Coremore API")
...
class ContactResponse(BaseModel):
    ...
    message: str = "Thank you. A Coremore partner will be in touch within one business day."


# ============ Routes ============
@api_router.get("/")
async def root():
    return {"message": "Coremore API — More with the Core."}
...
async def create_contact(payload: ContactSubmissionCreate):
    ...
    return ContactResponse(id=submission.id, submitted_at=submission.submitted_at)


@api_router.get("/contact", response_model=List[ContactSubmission])
async def list_contacts(limit: int = 50):
    items = await db.contact_submissions.find({}, {"_id": 0}).sort("submitted_at", -1).to_list(limit)
    for it in items:
